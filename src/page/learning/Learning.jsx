...
const Learning = () => {
      let imageurl = 'https://media.istockphoto.com/id/1320658267/photo/asian-college-student-is-making-a-presentation-in-front-of-projector-screen.jpg?s=1024x1024&w=is&k=20&c=9nTStY-6pBwo5zpBAQhbngYbNhccYYOSG_nNbcVQOFQ=';
      let title = 'Audit Law Forensics';
      let topic = 'A Standard Operating Procedure';
      let name = 'Roy Davis';
      let duration = '5 hrs';
      let rating = 4.4;
      let progress = 90;
                        ...
                        <div className='flex flex-col lg:flex-row  justify-between   pl-30 pr-20'>
                              ...
                              <div className='w-[78%] space-y-2'>
                                    ...
                                    <div className="rounded-lg grid grid-cols-3  gap-y-3 gap-x-3">
                                          <Card imageurl={imageurl} title={title} topic={topic} lecturer={name} duration={duration} rating={rating} progress={progress} />
                                          <Card imageurl={imageurl} title={title} topic={topic} lecturer={name} duration={duration} rating={rating} progress={progress} />
                                          <Card imageurl={imageurl} title={title} topic={topic} lecturer={name} duration={duration} rating={rating} progress={progress} />
                                          <Card imageurl={imageurl} title={title} topic={topic} lecturer={name} duration={duration} rating={rating} progress={progress} />
                                          <Card imageurl={imageurl} title={title} topic={topic} lecturer={name} duration={duration} rating={rating} progress={progress} />
